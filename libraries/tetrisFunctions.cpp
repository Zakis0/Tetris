...
void printField(char field[FIELD_HEIGHT][FIELD_WIDTH], Piece *heldPiece, array<int, 4> statArr) {
    ...
    char **field_ = createMatrix(FIELD_HEIGHT, FIELD_HEIGHT);

    fillMatrixWith(field_, FIELD_HEIGHT, FIELD_HEIGHT, SCREEN_EMPTY_PIXEL);

    for (int i = 0; i < FIELD_HEIGHT; ++i) {
        for (int j = 0; j < FIELD_WIDTH; ++j) {
            field_[i][j] = field[i][j];
        }
    }

    char **heldPieceField = createMatrix(PIECE_FIELD_HEIGHT, PIECE_FIELD_WIDTH);

    fillMatrixWith(heldPieceField, PIECE_FIELD_HEIGHT, PIECE_FIELD_WIDTH, SCREEN_EMPTY_PIXEL);

    pieceCord = heldPiece->getCords();
    arrayBiasToLeftTop(pieceCord);

    for (auto cord : pieceCord) {
        heldPieceField[cord.second][cord.first] = heldPiece->getPieceChar();
    }

    char **nextPieceField = createMatrix(NUM_OF_SHOWN_NEXT_PIECES * (PIECE_FIELD_HEIGHT + 1) - 1, PIECE_FIELD_WIDTH);
    fillMatrixWith(nextPieceField, NUM_OF_SHOWN_NEXT_PIECES * (PIECE_FIELD_HEIGHT + 1) - 1, PIECE_FIELD_WIDTH, SCREEN_EMPTY_PIXEL);
    ...
    for (int i = 0; i < NUM_OF_SHOWN_NEXT_PIECES; ++i) {
        ...
        arrayBiasToLeftTop(pieceCord);
        for (auto cord : pieceCord) {
            nextPieceField[cord.second + dY][cord.first] = piece->getPieceChar();
        }
        dY += PIECE_FIELD_HEIGHT + 1;
    }
    char **infoBlock = connectMatrixVertical(heldPieceField, nextPieceField, SCREEN_EMPTY_PIXEL, PIECE_FIELD_HEIGHT, PIECE_FIELD_WIDTH, NUM_OF_SHOWN_NEXT_PIECES * (PIECE_FIELD_HEIGHT + 1) - 1, PIECE_FIELD_WIDTH, NEXT_PIECE_FIELD_TOP_BIAS);

    char **screen = connectMatrixHorizontal(field_, infoBlock, SCREEN_EMPTY_PIXEL, FIELD_HEIGHT, FIELD_WIDTH, PIECE_FIELD_HEIGHT + NUM_OF_SHOWN_NEXT_PIECES * (PIECE_FIELD_HEIGHT + 1) - 1 + NEXT_PIECE_FIELD_TOP_BIAS, PIECE_FIELD_WIDTH, INFO_BLOCK_TOP_BIAS, INFO_BLOCK_LEFT_BIAS);

    printMatrix(screen, FIELD_HEIGHT, FIELD_WIDTH + PIECE_FIELD_WIDTH + INFO_BLOCK_LEFT_BIAS);

    cout << "Score: " << statArr[0] << "; NCL: " << statArr[1] << "; Tetris rate: " << getPercent(statArr[2], statArr[3]) << "%" << endl;
    cout << endl;
}

//void printFieldOld(char field[FIELD_HEIGHT][FIELD_WIDTH], Piece *heldPiece, array<int, 4> statArr) {
